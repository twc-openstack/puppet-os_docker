# == Class: os_docker::mistral::executor
#
  ...
  if $active_image_name {
    if $manage_service {
      $default_params = {
        ...
        command          => '/usr/bin/mistral-server --server executor --log-file=/var/log/mistral/mistral-executor.log',
        net              => 'host',
      ...
      $executor_resource = merge($default_params, $run_override)
      create_resources('::docker::run', { 'mistral-executor' => $executor_resource } )
    }

    docker::command { '/usr/bin/mistral-executor':
      command    => '/usr/bin/mistral-server --server executor --log-file=/var/log/mistral/mistral-executor.log',
      image      => "${active_image_name}:${active_image_tag}",
      net        => 'host',
      env        => $::os_docker::mistral::environment,
