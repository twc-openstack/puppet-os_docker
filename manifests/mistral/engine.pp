# == Class: os_docker::mistral::engine
#
  ...
  if $active_image_name {
    if $manage_service {
      $default_params = {
        ...
        command          => '/usr/bin/mistral-server --server engine --log-file=/var/log/mistral/mistral-engine.log',
        net              => 'host',
      ...
      create_resources('::docker::run', { 'mistral-engine' => $engine_resource } )
    }

    docker::command { '/usr/bin/mistral-engine':
      command    => '/usr/bin/mistral-server --server engine --log-file=/var/log/mistral/mistral-engine.log',
      image      => "${active_image_name}:${active_image_tag}",
      net        => 'host',
      env        => $::os_docker::mistral::environment,
