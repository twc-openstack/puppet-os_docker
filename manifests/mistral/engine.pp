# == Class: os_docker::mistral::conductor
#
  ...
  if $active_image_name {
    if $manage_service {
      $default_params = {
        ...
        command          => '/usr/bin/mistral-conductor',
        net              => 'host',
      ...
      create_resources('::docker::run', { 'mistral-conductor' => $conductor_resource } )
    }

    docker::command { '/usr/bin/mistral-conductor':
      command    => '/usr/bin/mistral-conductor',
      image      => "${active_image_name}:${active_image_tag}",
      net        => 'host',
      env        => $::os_docker::mistral::environment,
