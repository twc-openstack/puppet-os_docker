# == Class: os_docker::trove::taskmanager
#
  ...
  if $active_image_name {
    if $manage_service {
      $default_params = {
        ...
        # XXX - mfisch - do we need this? /usr/bin/trove-taskmanager --config-file=/etc/trove/trove.conf --log-dir=/var/log/trove --log-file=trove-taskmanager.log
        command          => '/usr/bin/trove-taskmanager',
        net              => 'host',
      ...
      create_resources('::docker::run', { 'trove-taskmanager' => $taskmanager_resource } )
    }

    docker::command { '/usr/bin/trove-taskmanager':
      command    => '/usr/bin/trove-taskmanager',
      image      => "${active_image_name}:${active_image_tag}",
      net        => 'host',
      env        => $::os_docker::trove::environment,
