  ...
  if $active_image_name {
    if $manage_service {
      $default_params = {
        ...
        # XXX - mfisch - do we need this? /usr/bin/trove-api --config-file=/etc/trove/trove.conf --log-dir=/var/log/trove --log-file=trove-api.log
        command          => '/usr/bin/trove-api',
        net              => 'host',
      ...
      create_resources('::docker::run', { 'trove-api' => $api_resource } )
    }

    docker::command { '/usr/bin/trove-api':
      command    => '/usr/bin/trove-api',
      image      => "${active_image_name}:${active_image_tag}",
      net        => 'host',
      env        => $::os_docker::trove::environment,
