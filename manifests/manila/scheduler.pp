# == Class: os_docker::manila::scheduler
#
  ...
  if $active_image_name {
    if $manage_service {
      $default_params = {
        ...
        command          => '/usr/bin/manila-scheduler',
        net              => 'host',
      ...
      create_resources('::docker::run', { 'manila-scheduler' => $scheduler_resource } )
    }

    docker::command { '/usr/bin/manila-scheduler':
      command => '/usr/bin/manila-scheduler',
      image   => "${active_image_name}:${active_image_tag}",
      net     => 'host',
      env     => $os_docker::manila::params::environment,
