# == Class: os_docker::nova::consoleauth
#
...
class os_docker::nova::consoleauth(
  ...
  if $active_image_name {
    if $manage_service {
      ...
      $consoleauth_resource = merge($default_params, $run_override)
      create_resources('::docker::run', { 'nova-consoleauth' => $consoleauth_resource } )
    }

    docker::command { '/usr/bin/nova-consoleauth':
      command => '/usr/bin/nova-consoleauth',
      image   => "${active_image_name}:${active_image_tag}",
      net     => 'host',
      volumes => [
