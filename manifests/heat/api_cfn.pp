...
class os_docker::heat::api_cfn(
  ...
  if $active_image_name {
    if $manage_service {
      ...
      $api_cfn_resource = merge($default_params, $run_override)
      create_resources('::docker::run', { 'heat-api-cfn' => $api_cfn_resource } )
    }

    docker::command { '/usr/bin/heat-api-cfn':
      command => '/usr/bin/heat-api-cfn',
      image   => "${active_image_name}:${active_image_tag}",
      net     => 'host',
      volumes => [
