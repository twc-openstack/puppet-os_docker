# == Class: os_docker::neutron::agents::metadata
#
...
class os_docker::neutron::agents::metadata(
  ...
  $command = join([
    ...
    '--config-file=/etc/neutron/metadata_agent.conf',
  ], ' ')
  ...
  if $active_image_name {
    if $manage_service {
      ...
      $metadata_agent_resource = merge($default_params, $run_override)
      create_resources('::docker::run', { 'neutron-metadata-agent' => $metadata_agent_resource } )
    }

    docker::command { '/usr/bin/neutron-metadata-agent':
      command    => $command,
      image      => "${active_image_name}:${active_image_tag}",
      net        => 'host',
      env        => $environment,
