...
class os_docker::ironic(
  ...
  docker::command { '/usr/bin/ironic-inspector-dbsync':
    command => '/usr/bin/ironic-inspector-dbsync',
    image   => "${active_image_name}:${active_image_tag}",
    net     => 'host',
    volumes => [
